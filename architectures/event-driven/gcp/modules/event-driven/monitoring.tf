...
resource "google_monitoring_alert_policy" "high_error_rate" {
  ...
  documentation {
    content = <<-EOT
      Cloud Run service ${google_cloud_run_v2_service.event_processor.name} in ${var.environment}
      is experiencing a high error rate (>${var.error_rate_threshold}%).
...
resource "google_monitoring_alert_policy" "old_unacked_messages" {
  ...
  conditions {
    ...
    condition_threshold {
      filter          = "resource.type = \"pubsub_subscription\" AND resource.labels.subscription_id = \"${google_pubsub_subscription.event_subscription.name}\" AND metric.type = \"pubsub.googleapis.com/subscription/oldest_unacked_message_age\""
      duration        = "300s"
      comparison      = "COMPARISON_GT"
  ...
  documentation {
    content = <<-EOT
      Pub/Sub subscription ${google_pubsub_subscription.event_subscription.name}
