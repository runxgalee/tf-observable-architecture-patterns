          ...
          widget = {
            ...
            xyChart = {
              dataSets = [{
                timeSeriesQuery = {
                  timeSeriesFilter = {
                    filter = "resource.type=\"pubsub_topic\" AND resource.labels.topic_id=\"${var.event_topic_name}\" AND metric.type=\"pubsub.googleapis.com/topic/send_request_count\""
                    aggregation = {
                      alignmentPeriod    = "60s"
          ...
          widget = {
            ...
            xyChart = {
              dataSets = [{
                timeSeriesQuery = {
                  timeSeriesFilter = {
                    filter = "resource.type=\"pubsub_subscription\" AND resource.labels.subscription_id=\"${var.event_subscription_name}\" AND metric.type=\"pubsub.googleapis.com/subscription/num_undelivered_messages\""
                    aggregation = {
                      alignmentPeriod  = "60s"
...
resource "google_logging_project_sink" "error_sink_bigquery" {
  ...
  lifecycle {
    precondition {
      condition     = var.error_log_dataset_id != ""
      error_message = "error_log_dataset_id is required when enable_error_log_sink is true."
    }
  }
}
